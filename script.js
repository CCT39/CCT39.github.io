...
function setQuestionMsg(date){
    if (date == 'Nov21'){
        ...
        return '問題集：<br><br>問題1：黑紅底色標題區的定位問題相關結構簡圖：<br>&lt;.grey /&gt;（灰色方塊）<br><br>&lt;.img&gt;（大圖區）<br>&emsp;&lt;.appeal&gt;（黑紅底色標題區）<br>&emsp;&emsp;&lt;.title /&gt;（紅底標題）<br>&emsp;&emsp;&lt;.txt /&gt;（黑底內文）<br>&emsp;&lt;/&gt;<br><br>&emsp;&lt;.explain /&gt;（說明內文）<br>&emsp;&lt;.icon-play /&gt;（播放鍵）<br>&lt;/&gt;<br><br>&emsp;思考：將內文.txt與大圖區.img切齊，預想也許標題區.appeal會隨著子層被移動而移動<br>&emsp;問題1-1：將黑底內文.txt設絕對定位，令其top為0與大圖容器切齊；而紅底標題卻留在原地，沒有隨著移動<br>&emsp;發現：黑底內文.txt該層單獨脫離空間自由移動，因此必須將絕對定位設給整個黑紅底色標題區.appeal，才能同時定位<br>&emsp;解決方案：設整個黑紅底色標題區.appeal為絕對定位。此時紅底標題上緣切齊大圖上緣，仍未達到目的<br><br>&emsp;思考：若設.appeal絕對定位，如何將.txt黑底切齊大圖？思考利用transform將.title往上移的可能性<br>&emsp;問題1-2：將.title設定transform: translate(0, -100%);使其往上平移一段相當於自身高度的距離；但原地會留出空白<br>&emsp;發現：使用translate移動.title後，仍然佔據著原先所在的空間，造成.title與.txt間留著一段相當於.title高度的距離<br>&emsp;解決方案：設定.title的position為inherit（繼承），使其可以拉著內文往上跑；如此達成高度未寫死，且內容可隨意增減，畫面仍然切齊的目的<br><br>思考：.title設定inherit實際上繼承了.appeal的絕對定位，不清楚是否算用掉1個或是2個扣打。此處假設用掉2個，只剩1個扣打<br>問題2：是否有辦法只使用1個絕對定位，便能將灰色方塊.grey、說明內文.explain和播放鍵.icon-play都移動到指定位置？<br>發現：整體寬度為960px的情況下，.explain貼齊左緣，而.grey貼齊右緣。考慮定位的方便性，嘗試將該兩物件設定相對定位<br>解決方案：.explain設相對定位，top為-10%，左或右為-160px；.grey設相對定位，top為180px，margin-left or right為auto；而icon-play使用絕對定位。綜上所述，該三物件僅使用1個絕對定位便達成目的<br><br>最終結構：<br>&lt;.grey /&gt;（相對 relative）<br><br>&lt;.img&gt;（相對 relative）<br>&emsp;&lt;.appeal&gt;（絕對 absolute）<br>&emsp;&emsp;&lt;.title /&gt;（繼承 inherit）<br>&emsp;&emsp;&lt;.txt /&gt;<br>&emsp;&lt;/&gt;<br><br>&emsp;&lt;.explain /&gt;（相對 relative）<br>&emsp;&lt;.icon-play /&gt;（絕對 absolute）<br>&lt;/&gt;';
    }
    else if (date == 'Dec15'){
        return '問題1：將子清單的開關（checkbox）放在header外面，使得CSS程式碼過於冗長<br>解決方案：將每個checkbox放在對應的item內，如此除了精簡CSS選取器長度以外，還可以透過選擇.sub-ctrls:checked ~ .sub-nav一次設定每個子清單的開關，而不需要個別選取並設定<br><br>問題2-1：手機版畫面時，將main-item（即主清單的每個項目）設flex，使得內部的超連結及箭頭橫排時，控制箭頭開啟sub-nav（display:none → block）後，sub-nav也加入該橫排<br>解決方案：將main-item的超連結及箭頭（即未開啟sub-nav時的可視部分）另外包成一個div，並改對該div設flex<br>問題2-2：手機版畫面時，當sub-nav開啟，其永遠出現在主清單各項目的最下方<br>解決方案：開啟後除了display:none → block以外，新增position: absolute → relative，使其占住空間，將主清單各其他項目往下推<br><br>問題3：桌機板畫面時，最右側的選單打開後，文字會超出邊緣<br>解決方案：將.sub-nav的寬度都固定，設定right: 0，如此每個子選單出現的位置都一樣，且寬度都大致與主選單的寬度相同';
    }
    else if (date == 'Dec19'){
        return 'Altis：<br><br>問題1：按轉向換圖，換了十幾張之後網頁會自動重新整理<br>解決方案：尚不清楚原因，但已不會發生該問題<br><br>問題2：如何做長按？<br>解決方案：對mousedown事件掛監聽，對換圖的方法設定setInterval()；再對mouseup事件掛監聽，使用clearInterval()清除間隔<br><br>LoL：<br><br>問題1：用querySelectorAll()選到每一張.card之後，使用forEach()無法選擇其子層元素，即.card-title和.card-body<br>解決方案：發現是觀念錯誤，對card使用forEach()，選到的是每一張.card本身，與其子層無關<br><br>問題2：在對卡片的forEach()內無法使用querySelectorAll()選到每一張.card-body<br>解決方案：每往下一層就用一個querySelector()；並且由於一張卡片只有一個.card-body，因此拿掉原有的All。如此可解決問題<br><br>問題3：沒有單獨一個屬性可以直接取得每張卡的中文名稱<br>解決方案：取得cardTitle.innerHTML之後，對其使用split("-")[1]分開後取得後者元素；並且由於字串冒頭留有空白，因此以trim()去除<br><br>問題4：heroArray中的每個元素都是物件，如何取得其屬性做成thead，以及每個物件每個屬性的值做成tbody？<br>解決方案：首先利用Object.keys(hero).map(key => hero[key])將物件轉換成陣列，保留屬性的值（但不太懂map()內的key代表什麼意思），再用forEach()取值並包成tr和整個tbody；接著以heroArray[0]為例取其屬性名稱並包裝成新陣列：let titleArray = []; Object.keys(heroArray[0]).forEach(x => titleArray.push(x));並將該陣列包成thead';
    }
